#!/bin/bash
#ffmpeg for windows arm
#windows_arm-build.sh requirements.
# MinGW
#
...
if (isstaticlib) then
    ...
    for fname in *.def; do
        ...
        if (isintobin) then
            ...
            lib \/machine:ARM \/def:$fname \/out:$abspath
            popd
        	rm -fv $binpath/$outname.exp
        	pushd $libpath
        else
            lib \/machine:ARM \/def:$fname \/out:$outname.lib
            rm -fv $outname.exp
        fi
    done
    popd
fi
echo "---- windows_arm-build.sh finished ----"
}
build_ffmpeg
