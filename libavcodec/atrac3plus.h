...
/** Global, bitstream-related VLC tables */
typedef struct ATRAC3PVlcTabs {
    /* VLC tables */
    VLC wl_vlc_tabs[4];
    VLC sf_vlc_tabs[8];
    VLC ct_vlc_tabs[4];
...
void ff_atrac3p_init_vlcs(ATRAC3PVlcTabs *vlc_tabs);
void ff_atrac3p_free_vlcs(ATRAC3PVlcTabs *vlc_tabs);
...
void ff_atrac3p_generate_tones(Atrac3pChanUnitCtx *ch_unit, int ch_num, int sb,
                               float *out);
void ff_atrac3p_power_compensation(Atrac3pChanUnitCtx *ctx, int ch_index,
                                   float *sp, int rng_index, int sb_num);
void ff_atrac3p_imdct(FFTContext *mdct_ctx, float *pIn, float *pOut,
                      int wind_id, int sb);
