...
av_cold void ff_atrac3p_init_wave_synth(void)
{
    ...
    /* generate first half of the Hann window */
    for (i = 0; i < 129; i++)
                        ...
                        int phase_shift, int reg_offset, float *out)
{
    ...
    for (wn = 0; wn < waves_info->num_wavs; wn++, wave_param++) {
        ...
        amp = amp_sf_tab[wave_param->amp_sf] *
              ((!synth_param->amplitude_mode) ? amp_mant_tab[wave_param->amp_index] : 1.0f);
...
void ff_atrac3p_generate_tones(Atrac3pChanUnitCtx *ch_unit, int ch_num, int sb,
                               float *out)
{
    float wavreg1[128] = { 0 }, wavreg2[128] = { 0 };
...
{
    int i;
    float *wind_1st_half, *wind_2nd_half;
    ...
    /* select MDCT window */
    switch (wind_id & 3) {
    case 0: /* simply sine window */
        wind_1st_half = ff_sine_128;
        wind_2nd_half = ff_sine_128;
        break;
    case 1: /* 1st half: simple sine window, 2nd half: steep window */
        wind_1st_half = ff_sine_128;
        wind_2nd_half = mdct_wind_steep;
        break;
    case 2: /* 1st half: steep window, 2nd half: simple sine window */
        wind_1st_half = mdct_wind_steep;
        wind_2nd_half = ff_sine_128;
        break;
    case 3: /* steep window */
        wind_1st_half = mdct_wind_steep;
    ...
    atrac3p_dsp.vector_fmul(pOut, pOut, wind_1st_half, ATRAC3P_MDCT_SIZE / 2);
    atrac3p_dsp.vector_fmul_reverse(&pOut[128], &pOut[128], wind_2nd_half,
                                    ATRAC3P_MDCT_SIZE / 2);
}
