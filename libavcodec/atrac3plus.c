        ...
        if (ch_num) {
            ...
            if (chan->num_coded_vals) {
                ...
                for (i = pos; i < chan->num_coded_vals; i++)
                    chan->qu_wordlen[i] =
                        min_val + (delta_bits ? get_bits(gb, delta_bits) : 0);
            }
        }
        break;
    case 2:
                                 ...
                                 AVCodecContext *avctx)
{
            ...
            if (weight_idx == 3) {
                ...
                for (i = num_long_vals; i < ctx->used_quant_units; i++)
                    chan->qu_sf_idx[i] = (chan->qu_sf_idx[i] + min_val +
                                          (delta_bits ? get_bits(gb, delta_bits) : 0)) & 0x3F;
            } else {
                num_long_vals = get_bits(gb, 5);
    ...
    case 2:
        if (ch_num) {
            ...
            vlc_tab = &vlc_tabs->sf_vlc_tabs[get_bits(gb, 2) + 4];
    ...
    case 3:
        if (ch_num) {
            ...
            vlc_tab    = &vlc_tabs->sf_vlc_tabs[vlc_sel];

            if (weight_idx == 3) {
                vlc_tab = &vlc_tabs->sf_vlc_tabs[vlc_sel + 4];
...
static int decode_quant_wordlen(GetBitContext *gb, Atrac3pChanUnitCtx *ctx,
                                int num_channels, ATRAC3PVlcTabs *vlc_tabs,
                                AVCodecContext *avctx)
{
    int ch_num, i, ret;
    ...
    for (ch_num = 0; ch_num < num_channels; ch_num++) {
        ...
        if ((ret = decode_channel_wordlen(gb, ctx, ch_num, vlc_tabs, avctx)) < 0)
            return ret;
    }
                                ...
                                AVCodecContext *avctx)
{
    ...
    for (ch_num = 0; ch_num < num_channels; ch_num++) {
        ...
        if ((ret = decode_channel_sf_idx(gb, ctx, ch_num, vlc_tabs, avctx)) < 0)
            return ret;
    }
                                     ...
                                     AVCodecContext *avctx)
{
    ...
    for (ch_num = 0; ch_num < num_channels; ch_num++) {
        ...
        if ((ret = decode_channel_code_tab(gb, ctx, ch_num, vlc_tabs, avctx)) < 0)
            return ret;
    }
    ...
    for (ch_num = 0; ch_num < num_channels; ch_num++) {
        ...
        for (qu = 0; qu < ctx->used_quant_units; qu++) {
            ...
            if (wordlen) {
                ...
                decode_qu_spectra(gb, tab, &vlc_tabs->spec_vlc_tabs[tab_index],
    ...
    case 1: /* variable-length coding */
        for (i = 0; i < coded_subbands; i++)
            chan->gain_data[i].num_points =
                get_vlc2(gb, vlc_tabs->gain_vlc_tabs[0].table,
                         vlc_tabs->gain_vlc_tabs[0].bits, 1);
        break;
    case 2:
        if (ch_num) { /* VLC modulo delta to master channel */
            for (i = 0; i < coded_subbands; i++) {
                delta = get_vlc2(gb, vlc_tabs->gain_vlc_tabs[1].table,
                                 vlc_tabs->gain_vlc_tabs[1].bits, 1);
                chan->gain_data[i].num_points =
                                      ...
                                      ATRAC3PVlcTabs *vlc_tabs)
{
    ...
    if (dst->num_points > 0)
        dst->lev_code[0] = get_vlc2(gb, vlc_tabs->gain_vlc_tabs[2].table,
                                    vlc_tabs->gain_vlc_tabs[2].bits, 1);

    for (i = 1; i < dst->num_points; i++) {
        delta = get_vlc2(gb, vlc_tabs->gain_vlc_tabs[3].table,
                         vlc_tabs->gain_vlc_tabs[3].bits, 1);
...
static inline void gainc_loc_mode1(GetBitContext *gb, Atrac3pChanUnitCtx *ctx,
                                   AtracGainInfo *dst, ATRAC3PVlcTabs *vlc_tabs)
{
    int i;
    VLC *tab;
    ...
    case 1:
        if (ch_num) {
            for (sb = 0; sb < coded_subbands; sb++) {
                ...
                delta = get_vlc2(gb, vlc_tabs->gain_vlc_tabs[10].table,
                                 vlc_tabs->gain_vlc_tabs[10].bits, 1);
                ...
                for (i = 1; i < dst->num_points; i++) {
                    ...
                    if (dst->lev_code[i] > dst->lev_code[i - 1]) {
                        ...
                        if (more_than_ref) {
                            delta =
                                get_vlc2(gb, vlc_tabs->gain_vlc_tabs[9].table,
                                         vlc_tabs->gain_vlc_tabs[9].bits, 1);
                        ...
                        tab   = more_than_ref ? &vlc_tabs->gain_vlc_tabs[7]
                                              : &vlc_tabs->gain_vlc_tabs[10];
            ...
            for (sb = 0; sb < coded_subbands; sb++)
                gainc_loc_mode1(gb, ctx, &chan->gain_data[sb], vlc_tabs);
        break;
    case 2:
        if (ch_num) {
            for (sb = 0; sb < coded_subbands; sb++) {
                ...
                if (dst->num_points > ref->num_points || get_bits1(gb))
                    gainc_loc_mode1(gb, ctx, dst, vlc_tabs);
            ...
            for (sb = 1; sb < coded_subbands; sb++) {
                ...
                delta = get_vlc2(gb, vlc_tabs->gain_vlc_tabs[6].table,
                                 vlc_tabs->gain_vlc_tabs[6].bits, 1);
                ...
                for (i = 1; i < dst->num_points; i++) {
                    ...
                    tab = &vlc_tabs->gain_vlc_tabs[(dst->lev_code[i] > dst->lev_code[i - 1]) *
                                                   2 + more_than_ref + 6];
                    delta = get_vlc2(gb, tab->table, tab->bits, 1);
                    if (more_than_ref)
                        dst->loc_code[i] = dst->loc_code[i - 1] + delta;
                             ...
                             AVCodecContext *avctx)
{
    ...
    for (ch_num = 0; ch_num < num_channels; ch_num++) {
        ...
        if (get_bits1(gb)) { /* gain control data present? */
            ...
            if ((ret = decode_gainc_npoints(gb, ctx, ch_num, coded_subbands, vlc_tabs, avctx)   < 0) ||
                (ret = decode_gainc_levels(gb, ctx, ch_num, coded_subbands, vlc_tabs, avctx)    < 0) ||
                (ret = decode_gainc_loc_codes(gb, ctx, ch_num, coded_subbands, vlc_tabs, avctx) < 0))
                return ret;
                               ...
                               ATRAC3PVlcTabs *vlc_tabs, AVCodecContext *avctx)
{
    ...
    case 1: /** variable-length coding */
        for (sb = 0; sb < ctx->waves_info->num_tone_bands; sb++)
            if (band_has_tones[sb])
                dst[sb].num_wavs =
                    get_vlc2(gb, vlc_tabs->tone_vlc_tabs[1].table,
                             vlc_tabs->tone_vlc_tabs[1].bits, 1);
        break;
    case 2: /** VLC modulo delta to master (slave only) */
        for (sb = 0; sb < ctx->waves_info->num_tone_bands; sb++)
            if (band_has_tones[sb]) {
                delta = get_vlc2(gb, vlc_tabs->tone_vlc_tabs[2].table,
                                 vlc_tabs->tone_vlc_tabs[2].bits, 1);
                                   ...
                                   ATRAC3PVlcTabs *vlc_tabs)
{
    ...
    if (!ch_num || !get_bits1(gb)) { /* mode 0: fixed-length coding */
        ...
        for (sb = 0; sb < ctx->waves_info->num_tone_bands; sb++) {
            ...
            for (i = 0; i < dst[sb].num_wavs; i++) {
                delta = get_vlc2(gb, vlc_tabs->tone_vlc_tabs[6].table,
                                 vlc_tabs->tone_vlc_tabs[6].bits, 1);
            ...
            if (ctx->waves_info->amplitude_mode)
                for (i = 0; i < dst[sb].num_wavs; i++)
                    ctx->waves_info->waves[dst[sb].start_index + i].amp_sf =
                        get_vlc2(gb, vlc_tabs->tone_vlc_tabs[3].table,
                                 vlc_tabs->tone_vlc_tabs[3].bits, 1) + 20;
            else
                ctx->waves_info->waves[dst[sb].start_index].amp_sf =
                    get_vlc2(gb, vlc_tabs->tone_vlc_tabs[4].table,
                             vlc_tabs->tone_vlc_tabs[4].bits, 1) + 24;
        }
        break;
    case 2: /** VLC modulo delta to master (slave only) */
        for (sb = 0; sb < ctx->waves_info->num_tone_bands; sb++) {
            if (!band_has_tones[sb] || !dst[sb].num_wavs)
                continue;
            for (i = 0; i < dst[sb].num_wavs; i++) {
                delta = get_vlc2(gb, vlc_tabs->tone_vlc_tabs[5].table,
                                 vlc_tabs->tone_vlc_tabs[5].bits, 1);
                             ...
                             AVCodecContext *avctx)
{
    ...
    ctx->waves_info->num_tone_bands =
        get_vlc2(gb, vlc_tabs->tone_vlc_tabs[0].table,
                 vlc_tabs->tone_vlc_tabs[0].bits, 1) + 1;
    ...
    for (ch_num = 0; ch_num < num_channels; ch_num++) {
        ...
        decode_tones_frequency(gb, ctx, ch_num, band_has_tones, vlc_tabs);
        decode_tones_amplitude(gb, ctx, ch_num, band_has_tones, vlc_tabs,
                               avctx);
                                   ...
                                   AVCodecContext *avctx)
{
    ...
    if ((ret = decode_quant_wordlen(gb, ctx, num_channels, vlc_tabs, avctx)) < 0)
        return ret;
    ...
    if ((ret = decode_scale_factors(gb, ctx, num_channels, vlc_tabs, avctx)) < 0)
        return ret;

    if ((ret = decode_code_table_indexes(gb, ctx, num_channels, vlc_tabs, avctx)) < 0)
        return ret;

    decode_spectrum(gb, ctx, num_channels, vlc_tabs, avctx);
    ...
    if ((ret = decode_gainc_data(gb, ctx, num_channels, vlc_tabs, avctx)) < 0)
        return ret;

    if ((ret = decode_tones_info(gb, ctx, num_channels, vlc_tabs, avctx)) < 0)
        return ret;
